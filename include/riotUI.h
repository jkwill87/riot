 ...
 * ARGUMENTS:   - The Windows struct containing the target ncurses WINDOWs
 * (struct Windows *win).*/
...
/* DESCRIPTION: The menu that comes up when 'C' is pressed; it displays all the
 *              levels and lets the user select ones that are unlocked.
 * ARGUMENTS:   - The game interface (struct Windows *gameInterface).
                - The list of maps (struct MapList *mapList).
                - Value that represents how many levels the user has unlocked*/
 ...
 *             - The map that will be loaded (struct Map* map).
 *             - List of Guards that gets drawn onto the map.*/
void drawLevel(struct Windows *windows, struct Map *map,  struct UnitList
*guards);


/* DESCRIPTION: Redraws the screen, this is for color functionality
 * ARGUMENTS:   - The body window that is drawn (WINDOW *body).
 *              - Map struct to know which level user is playing.
 *              - List of Guards (struct UnitList *guardList).
 *              - List of Inmates (struct UnitList *inmateList).
 *              - Path struct to print units along the path*/
 ...
 *              - Previous position of this inmate.*/
 ...
 * ARGUMENTS:   - A position of a unit as a single integer value.*/
